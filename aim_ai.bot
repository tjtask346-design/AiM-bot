...
# পিং সিস্টেমের জন্য
PING_CHAT_ID = 7518554107  # আপনার চ্যাট ID
PING_INTERVAL = 600  # 10 মিনিট = 600 সেকেন্ড
PING_MESSAGE = "Bot Is running /start"
...
TELEGRAM_TOKEN = "8300778474:AAGtUTHJuEV4M-z4PdQdSSkF_RZY4CyrL2Y"
...
# পিং সিস্টেম ফাংশন
async def ping_system(context: ContextTypes.DEFAULT_TYPE):
    """প্রতি ১০ মিনিট পরপর পিং মেসেজ পাঠায়"""
    try:
        await context.bot.send_message(
            chat_id=PING_CHAT_ID,
            text=PING_MESSAGE
        )
        logger.info(f"✅ Ping sent to chat ID: {PING_CHAT_ID} at {datetime.now()}")
    except Exception as e:
...
def main():
    ...
    # কমান্ড হ্যান্ডলার যোগ
    application.add_handler(CommandHandler("start", start))
    application.add_handler(CommandHandler("help", help_command))
    application.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, handle_message))

    # পিং সিস্টেম সেটআপ
    # বট চালু হওয়ার সাথে সাথে প্রথম পিং পাঠাবে
    application.job_queue.run_once(
        ping_system,
        when=5,  # 5 সেকেন্ড পর প্রথম পিং
        chat_id=PING_CHAT_ID,
        name="initial_ping"
    )
    
    # প্রতি ১০ মিনিট পরপর পিং পাঠানো
    application.job_queue.run_repeating(
        ping_system,
        interval=PING_INTERVAL,  # 600 সেকেন্ড = 10 মিনিট
        first=10,  # প্রথম পিং 10 সেকেন্ড পর
        chat_id=PING_CHAT_ID,
        name="regular_ping"
    )
    ...
    logger.info(f"✅ AiM Bot is ready and running!")
    logger.info(f"📱 Ping system activated - will send message every 10 minutes to chat ID: {PING_CHAT_ID}")
